...
class BayesianOptimization(Observable):
    ...
    def res(self):
        """
            # print(main_optimizer.res)

            [{'target': 510.0, 'params': {'x1': 2.9595770164590087, 'x2': 3.0211357270475534}}, 
            {'target': 710.0, 'params': {'x1': 5.0, 'x2': 0.5}}, 
            {'target': 678.0, 'params': {'x1': 4.40500667760561, 'x2': 2.8104827100556316}}]
        """
    ...
    def suggest(self, utility_function):
        ...
        if type(utility_function) == list:
            for acquisition in utility_function:
                # Finding argmax of the acquisition function.
                suggestion = acq_max(
                    ac=acquisition.utility,
                    gp=self._gp,
                    y_max=self._space.target.max(),
                    bounds=self._space.bounds,
                    random_state=self._random_state
                )
                result.append(self._space.array_to_params(suggestion))
    ...
    def _prime_subscriptions(self):
        if not any([len(subs) for subs in self._events.values()]):
            _MasterLogger = _get_default_MasterLogger(self._verbose)
            self.subscribe(Events.OPTMIZATION_START, _MasterLogger)
            self.subscribe(Events.OPTMIZATION_STEP, _MasterLogger)
            self.subscribe(Events.OPTMIZATION_END, _MasterLogger)
    ...
    def output_space(self, dirname, filename):
        """
        [Modified by HJ]
...
class DiscreteBayesianOptimization(BayesianOptimization):
    ...
    def __init__(self, f, prange, random_state=None, verbose=2, constraints=[], targetName=""):
        # Data structure containing the function to be optimized, the bounds of
        # its domain, and a record of the evaluations we have done so far
        self._pbounds = {item[0]: (item[1][:2]) for item in sorted(prange.items(), key=lambda x: x[0])}
        super(DiscreteBayesianOptimization, self).__init__(f, self._pbounds, random_state, verbose)
        ...
        kernel = Matern(length_scale=length_scale,
                        length_scale_bounds=(1e-04, 1e4),
                        nu=2.5) * \
                 ConstantKernel(1.0, (0.5, 5)) + \
                 WhiteKernel(noise_level=0.01,
                             noise_level_bounds=(1e-03, 1e3))
    ...
    def _prime_subscriptions(self):
        if not any([len(subs) for subs in self._events.values()]):
            _MasterLogger = _get_discrete_logger(self._verbose)
            self.subscribe(Events.OPTMIZATION_START, _MasterLogger)
            self.subscribe(Events.OPTMIZATION_STEP, _MasterLogger)
            self.subscribe(Events.OPTMIZATION_END, _MasterLogger)
            self.subscribe(Events.BATCH_END, _MasterLogger)
    ...
    def constrained_rng(self, n_points, bin=False):
        ...
        총 부피가 5ml 이상은 안되서 candidiate 생성할 때, constrained 거는 것
