...
class TargetSpace(object):
    ...
    def latin_sample(self ,n_samples):
        """
        ...
        x = lhs.generate(space, n_samples)
        # print(x)
        
        # if eval says reject, reject = true, break
        return np.array(x)

    def grid_sample(self ,n_samples):
        """
        Params
        ---------------
    ...
    # def random_sample(self ,n_samples, random_state):
    #     """
    #     :params n_sample (int): the number of sampling =( the number of samlping cycles * the number of batch_size)
    #     :params random_state (int) : random seed
    #     samplingrng is range of param  ex AgNO3 : [500, 3000], AgNO3 NaBH4[[500,3000],[500,3000]]
    #     x is samplinglist
    #     """
    ...
    #     x= space.rvs(n_samples, random_state)
...
class PropertySpace(TargetSpace):
    """
    [Modified by HJ]

    store property values in PropertySpace class which is similar with TargetSpace class
    ...
    """
    def __init__(self, pbounds, target_condition_dict):
        super(PropertySpace, self).__init__(target_func=None, pbounds=pbounds, random_state=None)
        property_list=[]
        for action, action_dict in target_condition_dict.items():
            aciton_property_list=list(action_dict["Ratio"].keys())
            property_list.extend(aciton_property_list)
        
        self._keys=sorted(pbounds) + property_list
