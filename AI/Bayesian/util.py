...
# -*- coding: utf-8 -*-
# ##
# @brief    [util.py] acquistion function
# @Inspiration
...
class UtilityFunction(object):
    ...
    if self.kind == 'es':
        return self._es(x, gp)
    if self.kind == 'third':
        return self._ei(x, gp, y_max, self.xi), self._ucb(x, gp, y_max, self.kappa), self._es(x, gp)
    """

    def __init__(self, kind, kappa=2.0, xi=0.0):
        """
        If UCB is to be used, a constant kappa is needed.

        third means "ei, poi, es"
        """
        self.kappa = kappa

        self.xi = xi

        if kind not in ['ucb', 'ei', 'poi', 'es', 'third']:
    ...
    @staticmethod
    def _es(x, gp):
        """Entropy Search (add new function)"""
        with warnings.catch_warnings():
            warnings.simplefilter("ignore")
...
def get_rnd_simplex(dimension, random_state_obj):
    '''
    ...
    :param dimension (int): how many grid do you divide?
    :param random_state_obj (np.random.RandomState()): RandomState's object

    :return :
    random_state=np.random.RandomState(1)
    t = random_state.uniform(0,1,5)
    t_1 = np.append(t, [0,1])
    t_1.sort()
    --> result : t_1 = [0.00000000e+00 1.14374817e-04 1.46755891e-01 3.02332573e-01
                        4.17022005e-01 7.20324493e-01 1.00000000e+00]
    
    return --> array([1.14374817e-04, 1.46641516e-01, 1.55576682e-01, 1.14689432e-01,
       3.03302489e-01, 2.79675507e-01])
    '''
    t = random_state_obj.uniform(0, 1, dimension - 1)
    t = np.append(t, [0, 1])
    t.sort()
...
def get_rng_complement(dimension, random_state_obj):
    """
    ...
    float, [0,1]
    """
    return 0.5 + get_rnd_simplex(dimension, random_state_obj)[0] * [-0.5, 0.5][np.random.randint(2)]
